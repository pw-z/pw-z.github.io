        ...
        for d_or_f in dir_list:
            ...
            try:
                ...
                if item.endswith('.jpg') or item.endswith('.JPG'):
                    ...
                    date_taken = get_field(img, "DateTimeOriginal")
                    print("DateTime: " + date_taken) # datetime str like 2014:05:11 12:20:39
                    date_taken = str(date_taken).replace(" ", "_").replace(":", "")
                    print("DateTimeFileNameStr: " + date_taken) # 20140511_122039
                    print("original filename: " + d_or_f)
                    print("rename with: " + date_taken+'.jpg')
                    img.close()

                    short_original_filename = "_"+d_or_f[:-4] if len(d_or_f) <= 13 else ""
                    # 同一秒拍摄多张照片，重命名冲突处理
                    try:
                        # rename files with timestamp
                        os.rename(item, os.path.join(current_path, f"{date_taken}{short_original_filename}.jpg"))
                    except FileExistsError as e:
                        os.rename(item, os.path.join(current_path, f"{date_taken}{short_original_filename}_{random.random()}.jpg"))
                    process_count += 1
